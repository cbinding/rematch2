# =============================================================================
# Package   : rematch
# Module    : TemporalEntityRecognizer.py
# Version   : 20220427
# Creator   : Ceri Binding, University of South Wales / Prifysgol de Cymru
# Contact   : ceri.binding@southwales.ac.uk
# Project   : ReMatch
# Summary   : TemporalEntityRecognizer class
# Imports   : json, spacy, os, fnmatch, argparse
# Example   : ner = TemporalEntityRecognizer("en");
...
class TemporalEntityRecognizer:
    ...
    def __load_token_patterns_from_directory(directory_name):
        ...
        for file_name in fnmatch.filter(file_names, "*.json"):
            ...
            token_patterns.extend(TemporalEntityRecognizer.__load_json_from_file(file_name_with_path))
    ...
    def get_entities(self, text):
        ...
        #nlp = self.__getNLP()
        # TODO: get language-specific pipeline
        #if(self._language == "fr"):
            #nlp = spacy.load("fr_core_news_sm")
        #elif(self._language == "no"):
            #nlp = spacy.load("nb_core_news_sm")
        #else:
        #nlp = spacy.blank(self._language) # but can't use POS with this (though not using any atm?)
        nlp = spacy.load("en_core_web_sm", disable = ['ner'])
        nlp.max_length = 2000000
        ...
        # pipeline component adding 'composite' entityRuler patterns incorporating atomic patterns (year spans, century spans that use dateprefix, datesuffix etc)
    ...
    def format_entities(text, ents, accept_format="application/json"):
        ...
        if accept_format.endswith("html"):
            formatted = TemporalEntityRecognizer.__renderHTML(text, ents)
        elif accept_format.endswith("tab-separated-values") or accept_format.endswith("tsv"):
            formatted = TemporalEntityRecognizer.__renderTSV(ents)
        elif accept_format.endswith("csv"):
            formatted = TemporalEntityRecognizer.__renderCSV(ents)
        else: # default application/json
            formatted = TemporalEntityRecognizer.__renderJSON(ents)
    ...
    def __renderTSV(ents):         	
        return TemporalEntityRecognizer.__renderDelimited(ents)
