...
class EntityPair:
    ...
    def __init__(self, ent1: Span = None, ent2: Span = None, rel_op: str = ""):
        self.rel_op = (rel_op or "").strip().lower()
        self.ent1 = ent1
        self.ent2 = ent2

    # scoring used to rank pairs based on relationship between pair elements
    @staticmethod
    ...
    def score(self):
        return self.get_score_for_rel_op(self.rel_op)

    def __str__(self):
        return f"{self.ent1.text} {self.rel_op} {self.ent2.text} ({self.score})"


class EntityPairs:

    def __init__(self, doc: Doc, rel_ops: list = []):
        self.doc = doc
        self.rel_ops = rel_ops
        self.pairs = self.get_pairs()


    def _get_noun_chunk_pairs(self) -> list:
        matched = dict()
    ...
    def _get_dependency_pairs(self) -> list:
    ...
    def _get_dependency_pairs_by_rel_op(self, rel_op: str = "") -> list:
        ...
        for match_id, token_ids in matches:
            ...
            for ent1, ent2 in itertools.product(periods, objects):
                id = f"{ent1.ent_id_}|{ent2.ent_id_}"
                matched[id] = EntityPair(ent1, ent2, rel_op)
    ...
    def _get_pairs(self) -> list:
        ...
        return sorted(best_pairs, key=lambda x: x.get("score", 0), reverse=True)
...
def find_period_object_pairs(nlp=None, input_text: str = "") -> dict:
    ...
    result["counts"] = get_entity_counts_by_id(doc)
...
def results_to_text_file(file_name: str="", results: dict={}):
    ...
    with open(file_name, "w") as text_file:
        ...
        for r in results.get("results", []):            
            ...
            counts = get_entity_counts_by_id(doc)
            for item in counts:                    
    ...
    with open(file_name, "w") as html_file:
        ...
        for r in results.get("results", []):  
            ...
            if(identifier.startswith("http")):
                html_file.write(f"<div><strong>Identifier:</strong> <a href='{identifier}'>{identifier}</a></div>")
            else:
                html_file.write(f"<div><strong>Identifier:</strong> {identifier}</div>")
            
            html_file.write("<h3>Text:</h3>") 
