...
class AatVocabularyAnnotator(BaseAnnotator):
    def __init__(self,
                 language="en",
                 entity_types=["ACTIVITY", "AGENT",
                               "MATERIAL", "OBJECT", "STYLEPERIOD"],
                 patterns=[]) -> None:
        ...
        for entity_type in list(map(lambda s: (s or "").strip().upper(), entity_types)):
            if (entity_type == "ACTIVITY"):
                self._pipeline.add_pipe("aat_activity_ruler", last=True)
            elif (entity_type == "AGENT"):
                self._pipeline.add_pipe("aat_agent_ruler", last=True)
            elif (entity_type == "MATERIAL"):
                self._pipeline.add_pipe("aat_material_ruler", last=True)
            elif (entity_type == "OBJECT"):
                self._pipeline.add_pipe("aat_object_ruler", last=True)
            elif (entity_type == "STYLEPERIOD"):
                self._pipeline.add_pipe("aat_styleperiod_ruler", last=True)
            else:
                warnings.warn(
    ...
    def _to_html(doc):
        ...
        output = super(AatVocabularyAnnotator, AatVocabularyAnnotator)._to_html(
            doc, options=options)
