...
Example :   nlp.add_pipe("aat_styleperiod_ruler", last=True)           
...
def create_aat_styleperiod_ruler(nlp, name="aat_styleperiod_ruler", patterns=patterns_en_AAT_STYLEPERIODS):
    return PatternRuler(nlp, name, patterns)

# test the AAT_StylePeriodRuler class
if __name__ == "__main__":
    nlp = spacy.load("en_core_web_sm", disable=['ner'])
    nlp.add_pipe("aat_styleperiod_ruler", last=True)
    text = '''
    Aside from three residual flints, none closely datable, the earliest remains comprised a small assemblage of Roman pottery \
    and ceramic building styleperiod, also residual and most likely derived from a Roman farmstead found immediately to the north \
    within the Phase II excavation area. A single sherd of Anglo-Saxon grass-tempered pottery was also residual. The earliest \
    features, which accounted for the majority of the remains on site, relate to medieval agricultural styleperiod focused within \
