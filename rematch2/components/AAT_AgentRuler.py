...
Example :   nlp.add_pipe("aat_agent_ruler", last=True)           
...
from ..spacypatterns import patterns_en_AAT_AGENTS
from .PatternRuler import PatternRuler


@Language.factory("aat_agent_ruler")
def create_aat_agent_ruler(nlp, name="aat_agent_ruler", patterns=patterns_en_AAT_AGENTS):
...
if __name__ == "__main__":
    ...
    nlp.add_pipe("aat_agent_ruler", last=True)
    text = '''
    ...
    features, which accounted for the majority of the remains on site, relate to medieval agricultural agent focused within \
