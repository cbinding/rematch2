...
Example :   nlp.add_pipe("aat_material_ruler", last=True)           
...
from ..spacypatterns import patterns_en_AAT_MATERIALS
from .PatternRuler import PatternRuler


@Language.factory("aat_material_ruler")
def create_aat_material_ruler(nlp, name="aat_material_ruler", patterns=patterns_en_AAT_MATERIALS):
    return PatternRuler(nlp, name, patterns)

# test the AAT_MaterialRuler class
if __name__ == "__main__":
    nlp = spacy.load("en_core_web_sm", disable=['ner'])
    nlp.add_pipe("aat_material_ruler", last=True)
    text = '''
    ...
    features, which accounted for the majority of the remains on site, relate to medieval agricultural material focused within \
